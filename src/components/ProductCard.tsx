...
function StarRating({ value, maxStars = 5, iconSize = 16, color = "#0d9488" }: { value: number; maxStars?: number; iconSize?: number; color?: string }) {
  return (
    <div className="flex">
      {Array.from({ length: maxStars }).map((_, i) => {
        ...
        return (
          <svg
            ...
            height={iconSize}
            viewBox="0 0 24 24"
            fill={filled || partial ? color : "none"}
            stroke={color}
            strokeWidth="2"
          >
            ...
            {partial && (
              <polygon 
                points="12,2 15.09,8.26 22,9.27 17,14.14 18.18,21.02 12,17.77 12,2"
                fill={color}
              />
            )}
          </svg>
